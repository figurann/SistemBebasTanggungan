...
<body>
    <div class="container">
        ...
        <div class="verification-card">
            ...
            <table id="dataTable" class="data-table">
                ...
                <tbody>
                    ...
                    $query = "SELECT m.nama, m.NIM, t.jumlah_skkm, t.status, t.ID
                             FROM pengguna.Mahasiswa m JOIN 
                             pengguna.jumlahSKKM t ON m.NIM = t.NIM
                             ";
                    ...
                    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                        ...
                        <tr data-id="<?= htmlspecialchars($row['ID']) ?>">
                            ...
                            <td><?= htmlspecialchars($row['jumlah_skkm']) ?></td>
        ...
        function verifikasi(id) {
            if(confirm('Apakah Anda yakin ingin memverifikasi SKKM ini?')) {
                fetch('proses_verifikasi_skkm.php', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if(data.success) {
                        ...
                        alert('SKKM berhasil diverifikasi!');
                        location.reload();
                    } else {
                        alert('Gagal memverifikasi SKKM!');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    alert('Terjadi kesalahan saat memverifikasi SKKM!');
                });
            }
        }

        function tolakDokumen(id) {
            if(confirm('Apakah Anda yakin ingin menolak SKKM ini?')) {
                fetch('tolak_skkm.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                ...
                .then(data => {
                    if(data.success) {
                        ...
                        statusCell.style.color = "green";
                        buttonCell.innerHTML = '';
                        alert('SKKM telah ditolak karena tidak memenuhi kriteria!');
                        location.reload();
                    } else {
                        alert('Gagal menolak SKKM!');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    alert('Terjadi kesalahan saat menolak SKKM!');
                });
            }
        }
    </script>
